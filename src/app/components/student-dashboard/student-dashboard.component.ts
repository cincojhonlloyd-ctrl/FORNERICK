...
export class StudentDashboardComponent implements OnInit {
    ...
    loadStudentInfo() {
        // 0. Check URL Params for Auto-Login (Priority)
    ...
    loadNotifications() {
        ...
        this.notificationService.getNotifications(this.studentId).subscribe(notifs => {
    ...
    async loginWithId(studentId: string) {
        ...
        try {
            ...
            this.libraryService.getStudents().subscribe(students => {
    ...
    loadMyBorrows() {
        ...
        this.libraryService.getStudentBorrows(this.studentId).subscribe(borrows => {
            this.myBorrows = borrows;
            this.calculateTotalFines();
        });
    }

    async calculateTotalFines() {
        if (!this.studentId) return;
        try {
            this.totalFines = await this.libraryService.getStudentTotalFines(this.studentId);
            this.canBorrow = await this.libraryService.canStudentBorrow(this.studentId);
        } catch (err) {
    ...
    async requestBorrow(book: Book) {
        ...
        if (!this.canBorrow) {
            alert(`You have unpaid fines totaling ₱${this.totalFines.toFixed(2)}. Please settle your fines before borrowing.`);
            return;
        }
    ...
    getOverdueBorrows(): BorrowRecord[] {
        return this.myBorrows.filter(b => b.isOverdue && b.status === 'approved');
    }

    getLostBorrows(): BorrowRecord[] {
        return this.myBorrows.filter(b => b.status === 'lost');
    }

    getUnpaidFines(): BorrowRecord[] {
        return this.myBorrows.filter(b => b.fineStatus === 'unpaid');
    }
}
